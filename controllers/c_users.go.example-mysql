...
func GetUserById(w http.ResponseWriter, r *http.Request) {
	...
	if len(params["id"]) == 0 {
		...
		if err == nil {
			...
			// Database Query
			rows, err := utils.DB.Query("SELECT * FROM users WHERE id=? LIMIT 1", userID)
			if err != nil {
				log.Print(err)
			}
			defer rows.Close()
...
func AddUser(w http.ResponseWriter, r *http.Request) {
	...
	// Database Query
	_, err := utils.DB.Exec("INSERT INTO users (name, email) VALUE (?, ?)", user.Name, user.Email)
	if err != nil {
		log.Print(err)
	}
...
func PutUserById(w http.ResponseWriter, r *http.Request) {
	...
	if len(params["id"]) == 0 {
		...
		if err == nil {
			...
			// Database Query
			_, err := utils.DB.Exec("UPDATE users SET name=?, email=? WHERE id=? LIMIT 1", user.Name, user.Email, userID)
			if err != nil {
				log.Print(err)
			}
...
func DelUserById(w http.ResponseWriter, r *http.Request) {
	...
	if len(params["id"]) == 0 {
		...
		if err == nil {
			...
			// Database Query
			_, err := utils.DB.Query("DELETE FROM users WHERE id=? LIMIT 1", userID)
			if err != nil {
				log.Print(err)
			}
